...
后续文章会介绍，各种 `Informer`（如 `Informer、IndexInformer、SharedInformer、SharedIndexInformer`）的初始化函数依次创建 `knownObjects` 缓存、`DeltaFIO` 和 [`controller`](4.controller-informer.md)。`controller` 再将 `DeltaFIFO` 传给 [Reflector](3.reflector.md)，**Reflector 的 `ListAndWatch()` 方法是 DeltaFIFO 的生产者**：

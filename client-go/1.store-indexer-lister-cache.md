...
这两种 `Lister` 都是基于通用 `Indexer` 接口实现，可以通过对象名称、命名空间、标签选择器等查询条件 Get 或 List 对象。
两者的差别在于 `GenericLister` 不限 Namespace，而 `GenericNamespaceLister` 是只能 Get/List Indexer 中特定 Namespace 的对象。
...
```
+ 参入的 resource，只用于在告警错误信息中显示 Group 和 Resource 名称。

实际开发时，一般用 codegen 工具为用户定义的类型如 AolDeploy 自动生成**对象类型(非上面通用的 runtime.Object 类型)相关的 Informer 和 Lister**，而且可以从对象的 Informer 获取对象的 Lister。
